...
function Navigation({session,onNavigate}:{session:Session|null;onNavigate?:()=>void}){
 ...
 return <nav className="space-y-1">
   {links.filter(([, , ,roles])=>session
   &&roles.some(r=>r===session.role)).map(([Icon,label,href])=>{const active=href==="/admin"
   ?pathname===href
...
export function AdminShell({children,session,notifications}:{children:React.ReactNode;session:Session|null;notifications:Summary}){
 ...
 if(["/admin/login",
   ...
   "/admin/reset-password"].some(r=>pathname.startsWith(r))) return <>{children}</>;
 return <div className="min-h-screen bg-[#f5f6f8] xl:grid xl:grid-cols-[248px_minmax(0,1fr)] 2xl:grid-cols-[268px_minmax(0,1fr)]">
  <aside className="hidden min-h-screen border-r border-slate-200 bg-white xl:sticky xl:top-0 xl:flex xl:h-screen xl:flex-col">
   <div className="border-b border-slate-200 px-5 py-5">
     <SiteLogo/>
     <p className="mt-2 text-xs font-medium text-slate-400">Enterprise workspace</p>
   </div>
   <div className="flex-1 overflow-y-auto px-3 py-5">
     <p className="px-3 pb-3 text-[11px] font-semibold uppercase tracking-[.16em] text-slate-400">Workspace</p>
   ...
   <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur-xl">
    <div className="flex h-[68px] items-center justify-between gap-3 px-3 sm:px-5 lg:px-6 xl:px-8">
     <div className="flex items-center gap-3">
       ...
       className="grid h-10 w-10 place-items-center rounded-lg border border-slate-300 xl:hidden" 
       aria-label="Open navigation">
       <Menu size={19}/>
     </button>
       <div>
         <p className="text-sm font-semibold text-slate-950">Rennovex Admin</p>
       <p className="hidden text-xs text-slate-500 sm:block">Manage content, enquiries, and operations</p>
       </div>
     </div>
     <div className="flex items-center gap-2">
       ...
       <Link 
       href="/" 
       className="grid h-10 w-10 place-items-center rounded-lg border border-slate-300 text-slate-600 hover:bg-slate-50" 
  ...
  <AnimatePresence>
    {open&&<><button 
    className="fixed inset-0 z-50 bg-slate-950/45 xl:hidden" 
    onClick={()=>setOpen(false)} 
    aria-label="Close navigation"/><motion.aside 
    initial={{x:"-100%"}} 
    animate={{x:0}} 
    exit={{x:"-100%"}} 
    transition={{duration:.22}} 
    className="fixed inset-y-0 left-0 z-[60] flex w-[86vw] max-w-[320px] flex-col bg-white shadow-2xl xl:hidden">
    ...
    <div className="border-t border-slate-200 p-4">
      <LogoutButton onBeforeLogout={()=>setOpen(false)}/>
    </div>
    </motion.aside></>}
  </AnimatePresence>
 </div>
