...
export function AdminTrendChart({ data }: { data: TrendPoint[] }) {
  const maxValue = Math.max(1, 
    ...data.flatMap((point) => [point.messages, 
    point.quotes, 
    point.bookings]));
          ...
          <div 
            ...
            <div className="flex h-44 items-end lg:h-52 justify-center gap-1 sm:gap-1.5">
              {[
                ...
                const height = numericValue === 0 ? 4 : Math.max(12, 
                  Math.round((numericValue / maxValue) * 100));
                return <div 
                  key={index} 
                  title={`${numericValue}`} 
                  className={`w-2.5 rounded-t-md transition-all sm:w-4 ${className}`} 
                  style={{ height: `${height}%`, 
                  opacity: numericValue === 0 
                    ? 0.18 
                    : 1 }} />;
              })}
            </div>
            <p className="mt-3 truncate text-center text-[10px] font-semibold uppercase tracking-wide text-slate-400 sm:text-xs">
              {point.label}
            </p>
          </div>
        ))}
