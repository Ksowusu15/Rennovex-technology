  ...
  if (disabled) {
    return (
      <button
        type="button"
        disabled
        className="inline-flex w-full items-center justify-center gap-2 whitespace-nowrap rounded-xl bg-slate-100 px-3 py-2 text-sm font-semibold text-slate-400 disabled:cursor-not-allowed"
  ...
  if (!confirming) {
    return (
      <button
        ...
        className="inline-flex w-full items-center justify-center gap-2 whitespace-nowrap rounded-xl bg-red-50 px-3 py-2 text-sm font-semibold text-red-700 transition hover:bg-red-100"
      >
        <Trash2 size={15} />
        Delete
      </button>
  ...
  return (
    <div className="rounded-xl border border-red-200 bg-red-50 p-3 sm:col-span-2">
      <p className="text-xs font-semibold leading-5 text-red-800">
        Delete {userName}? This permanently removes the account and its active
