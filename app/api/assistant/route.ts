...
function replyFor(message: string) {
  ...
  if (/human|agent|person|team|call me|contact me/.test(text)) return { reply: "I’ve marked this conversation for the Rennovex team. Please share your name and email so they can follow up promptly.", needsReply: true };
  if (/price|cost|budget|quote|estimate/.test(text)) return { reply: "I can help prepare a project estimate. Typical pricing depends on scope, integrations, content, and timeline. Choose ‘Request a quote’ below and I’ll collect the details our team needs.", action: "quote" };
  if (/book|meeting|consult|appointment|schedule/.test(text)) return { reply: "You can book a consultation directly. Choose ‘Book consultation’ and select your preferred date and project area.", action: "booking" };
  if (/ecommerce|e-commerce|online store|shop/.test(text)) return { reply: "For an online store, we can provide product management, secure payments, order tracking, customer accounts, analytics, and an admin dashboard. Would you like a quote or a consultation?", action: "quote" };
...
export async function POST(request: Request) {
  const throttle = rateLimit(request, "assistant", 30, 600000);
  ...
  try {
    ...
    const session = input.sessionId
      ? await prisma.chatSession.update({
          ...
          data: {
            ...
            messages: { create: [{ sender: "VISITOR", content: input.message }, { sender: "ASSISTANT", content: answer.reply }] },
          },
        })
    ...
    console.error(error);
    return NextResponse.json({ error: "I couldn’t send that message. Please try again." }, { status: 400 });
  }
}
