...
function csv(value: unknown) { return `"${String(value 
  ?? "").replaceAll('"', 
  '""')}"`; }
...
export async function GET(request: NextRequest) {
  ...
  const rows = archived
    ...
    : await prisma.auditLog.findMany({ include: { user: { select: { email: true } } }, 
  ...
  const lines = rows.map((raw:any) => {
    ...
    return [
      isArchived 
        ? raw.originalCreatedAt.toISOString() 
  ...
  return new NextResponse([header.map(csv).join(","), 
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename=rennovex-${archived 
        ? "archived-" 
        : ""}activity-logs-${new Date().toISOString().slice(0,
        10)}.csv`,
