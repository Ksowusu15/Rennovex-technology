...
export async function POST(request: Request) {
  const throttle = rateLimit(request, "forgot-password", 5, 900000);
  ...
  try {
    ...
    if (process.env.RESEND_API_KEY && process.env.ADMIN_FROM_EMAIL) {
      ...
      if (!response.ok) {
        ...
        console.error("Resend password-reset email failed:", details);
