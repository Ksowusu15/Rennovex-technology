...
export async function POST(request:Request){
  ...
  try{const data=schema.parse(await request.json());
    await prisma.newsletterSubscriber.upsert({where:{email:data.email},
    update:{name:data.name
      ||undefined,
    isSubscribed:true},
    create:data});
    return NextResponse.json({ok:true});}catch{return NextResponse.json({error:"Enter a valid email address."},
    {status:400});}}
