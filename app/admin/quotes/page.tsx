...
export default async function QuotesPage({ searchParams }: { searchParams: Promise<{ edit?: string; success?: string }> }) {
  ...
  const edit = params.edit 
    ? await prisma.quoteRequest.findUnique({ where: { id: params.edit } }) 
    : null;
  ...
  return <div className="space-y-6">
    ...
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <h1 className="mt-2 text-3xl font-bold">Quote requests</h1>
        <p className="mt-2 text-slate-600">Review, update, qualify, and remove project enquiries.</p>
      </div>
      <a 
        href="/api/admin/exports?type=quotes" 
    ...
    {edit && <form 
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
        <p className="text-xs font-bold uppercase tracking-wider text-blue-700">Editing</p>
    ...
    <div className="overflow-x-auto rounded-3xl border border-slate-200 bg-white shadow-sm">
      ...
      <ConfirmDeleteButton 
      compact 
      message={`Delete quote ${i.reference}? This cannot be undone.`}/>
      </form>
      </div>
      </td>
      </tr>)}
      ...
      {items.length===0&&<div className="py-14 text-center">
        <FileText className="mx-auto text-slate-300"/>
      <p className="mt-3 text-slate-500">No quote requests yet.</p>
      </div>}
    </div>
  </div>;
}
