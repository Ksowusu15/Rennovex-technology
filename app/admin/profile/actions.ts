...
export async function updateProfile(formData: FormData) {
  ...
  await prisma.user.update({ where: { id: session.userId }, 
    data: { name } });
...
export async function changePassword(formData: FormData) {
  ...
  await prisma.user.update({ where: { id: session.userId }, 
    data: { password: await bcrypt.hash(newPassword, 
    12) } });
  await prisma.authSession.updateMany({ where: { userId: session.userId, 
    id: { not: session.sessionId }, 
    revokedAt: null }, 
    data: { revokedAt: new Date() } });
  await writeAudit("PASSWORD_CHANGE", 
    "AUTH", 
    session.userId, 
    "Administrator changed password; other device sessions were ended");
...
export async function updateSecurityPreferences(formData: FormData) {
  ...
  await writeAudit(
    ...
    session.userId,
    `New-device login alerts ${emailNewDeviceAlerts 
      ? "enabled" 
      : "disabled"}`,
  );
