...
export async function saveTestimonial(fd: FormData) {
  ...
  const existing = id ? await prisma.testimonial.findUnique({ where: { id } }) : null;
  let imageUrl = existing?.imageUrl ?? null;
  const file = fd.get("image");

  if (file instanceof File && file.size > 0) {
    if (!TYPES.has(file.type) || file.size > MAX) throw new Error("Use a JPG, PNG, WEBP or AVIF image up to 8 MB.");
    const uploaded = await uploadImage(file, "testimonials");
    const old = imageUrl;
    imageUrl = uploaded.secure_url;
    if (old) await deleteImage(old).catch(() => undefined);
  } else if (String(fd.get("removeImage") || "") === "1" && imageUrl) {
    const old = imageUrl; imageUrl = null;
  ...
  const data = {
    ...
    status: String(fd.get("status") || "PUBLISHED") as "DRAFT" | "PUBLISHED" | "ARCHIVED",
  };
  ...
  await writeAudit(id ? "UPDATE" : "CREATE", "TESTIMONIAL", saved.id, saved.name);
  revalidatePath("/");
...
export async function deleteTestimonial(fd: FormData) {
  ...
  await writeAudit("DELETE", "TESTIMONIAL", item.id, item.name);
  revalidatePath("/");
  revalidatePath("/admin/testimonials");
}
