...
export default async function Page({ searchParams }: { searchParams: Promise<{ edit?: string; new?: string }> }) {
  ...
  const items = await prisma.testimonial.findMany({ orderBy: [{ order: "asc" }, 
    { createdAt: "desc" }] });
  const edit = q.edit 
    ? await prisma.testimonial.findUnique({ where: { id: q.edit } }) 
    : null;
  ...
  return <><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
    <div>
      ...
      <h1 className="mt-3 text-3xl font-bold text-slate-950">Client Testimonials</h1>
    </div>
    <Link 
      href="?new=1" 
  ...
  {show && <form 
    ...
    <div className="grid gap-5 md:grid-cols-2">
      ...
      <label className="label">Role / Company
        <input 
        className="field" 
        name="role" 
        required 
    ...
    <label className="label">Testimonial
      <textarea 
  ...
  <div className="mt-7 overflow-x-auto rounded-2xl border border-slate-200 bg-white">
    ...
    <tbody>
      ...
      <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-full bg-slate-100">
        ...
        alt={item.name} 
        fill 
        className="object-cover"/> : <div className="grid h-full place-items-center font-bold text-slate-400">
