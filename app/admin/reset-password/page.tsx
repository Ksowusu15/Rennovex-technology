...
export default async function ResetPasswordPage({
  ...
  if (/^[a-f0-9]{64}$/i.test(token)) {
    try {
      ...
      console.error("Unable to validate password-reset token:", 
        error);
    }
  }
    ...
    <section className="relative grid min-h-screen place-items-center overflow-hidden bg-slate-50 px-4 py-10">
      ...
      <div className="relative w-full">
        {valid ? (
          ...
          <div className={`
  mx-auto w-full max-w-[460px] rounded-[1.75rem] border border-amber-200
  bg-white p-6 text-center shadow-2xl shadow-slate-950/10 sm:p-8
`}>
