...
import { removeUpload, saveUpload } from "@/lib/admin";
...
function googleMapsEmbed(formData: FormData) {
  const raw = String(formData.get("googleMapsEmbed") || "").trim();
  if (!raw) return null;

  const iframeSrc = raw.match(/src=["']([^"']+)["']/i)?.[1];
  const value = (iframeSrc || raw).replace(/&amp;/g, "&").trim();

  try {
    const url = new URL(value);
    const allowed = url.hostname === "www.google.com" || url.hostname === "google.com" || url.hostname === "maps.google.com";
    const isEmbed = url.pathname.includes("/maps/embed") || url.pathname.includes("/maps/d/embed");
    return allowed && isEmbed ? url.toString() : null;
  } catch {
    return null;
  }
}
...
export async function saveSettings(formData: FormData) {
  ...
  const uploadedLogo = await saveUpload(formData.get("logo") as File | null, "branding");
  if (uploadedLogo && existing.logoUrl) await removeUpload(existing.logoUrl);

  await updateSiteSettings({
    companyName: String(formData.get("companyName") || "Rennovex Technology").trim(),
  ...
  await writeAudit("UPDATE", "SETTINGS", "main", "Company, contact, map, social and SEO settings updated");
...
export async function removeLogo() {
  ...
  await writeAudit("DELETE", "LOGO", "main", "Company logo removed");
  revalidatePath("/", "layout");
